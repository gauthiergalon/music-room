...
pub async fn transfer_ownership(
    ...
    if let Some(tx) = state.active_rooms.read().await.get(&room_id) {
        let _ = tx.tx.send(WsEvent::UserOwnershipTransferred {
...
async fn handle_socket(
    ...
    let (tx, users_list) = {
        ...
        let list: Vec<crate::dtos::ws::UserInfo> = room
            .users
            .iter()
    ...
    let _ = tx.send(WsEvent::UserJoin {
        user_id,
        ...
        while let Some(Ok(Message::Text(text))) = receiver.next().await {
            if let Ok(event) = serde_json::from_str::<WsEvent>(&text) {
                let is_authorized = match &event {
                    ...
                    | WsEvent::Pause { .. }
                    | WsEvent::SeekTo { .. }
                    | WsEvent::NextTrack { .. } => is_owner,
                    WsEvent::QueueAdd { .. }
                    | WsEvent::QueueRemove { .. }
                    | WsEvent::QueueReorder { .. } => true,
                    _ => false,
                };
