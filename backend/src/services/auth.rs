use crate::{
	...
	models::user::NewUser,
	repositories::{refresh_tokens as refresh_tokens_repo, reset_tokens as reset_tokens_repo, users as users_repo},
...
async fn store_refresh_token(pool: &PgPool, user_id: Uuid) -> Result<String, AppError> {
	...
	refresh_tokens_repo::create(pool, NewRefreshToken { token_hash: token_pair.hash, user_id, expires_at }).await?;
...
pub async fn login(pool: &PgPool, jwt_secret: &str, email: &str, password: &str) -> Result<(String, String), AppError> {
	let email = email.trim().to_lowercase();
	let user = users_repo::find_by_email(pool, &email).await?.ok_or(AppError::Unauthorized(ErrorMessage::InvalidCredentials))?;
...
pub async fn refresh(pool: &PgPool, jwt_secret: &str, token: &str) -> Result<(String, String), AppError> {
	let token_hash = TokenPair::hash(token);
	let stored = refresh_tokens_repo::delete_and_return(pool, token_hash).await?.ok_or(AppError::Unauthorized(ErrorMessage::TokenInvalid))?;
...
pub async fn forgot_password(pool: &PgPool, email: &str) -> Result<(), AppError> {
	...
	let user_model = users_repo::find_by_email(pool, &email_address).await?.ok_or(AppError::NotFound(ErrorMessage::UserNotFound))?;

	if reset_tokens_repo::find_valid_by_user_id(pool, user_model.id).await?.is_some() {
...
pub async fn reset_password(pool: &PgPool, token: &str, new_password: &str) -> Result<(), AppError> {
	...
	let stored = reset_tokens_repo::delete_and_return(&mut *tx, token_hash).await?.ok_or(AppError::Unauthorized(ErrorMessage::TokenInvalid))?;
