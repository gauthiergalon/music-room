...
pub async fn get_me(pool: &PgPool, user_id: Uuid) -> Result<User, AppError> {
	users_repo::find_by_id(pool, user_id).await?.ok_or(AppError::NotFound(ErrorMessage::UserNotFound))
}
...
pub async fn update_password(pool: &PgPool, user_id: Uuid, current_password: &str, new_password: &str) -> Result<(), AppError> {
	let user = users_repo::find_by_id(pool, user_id).await?.ok_or(AppError::NotFound(ErrorMessage::UserNotFound))?;
...
pub async fn confirm_email(pool: &PgPool, token: &str) -> Result<(), AppError> {
	...
	let stored_token = email_tokens_repo::delete_and_return(&mut *tx, token_hash).await?.ok_or(AppError::Unauthorized(ErrorMessage::TokenInvalid))?;
...
pub async fn send_email_confirmation_email(pool: &PgPool, user_id: Uuid) -> Result<(), AppError> {
	...
	if email_tokens_repo::find_valid_by_user_id(pool, user_id).await?.is_some() {
