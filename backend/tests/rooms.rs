...
async fn test_create(pool: PgPool) {
	...
	let res = server.post("/rooms").add_header(axum::http::header::AUTHORIZATION, format!("Bearer {}", token)).await;
...
async fn test_get_room(pool: PgPool) {
	...
	let create_res = server.post("/rooms").add_header(axum::http::header::AUTHORIZATION, format!("Bearer {}", token)).await;
	let room = create_res.json::<TestRoomResponse>();

	let res = server.get(&format!("/rooms/{}", room.id)).add_header(axum::http::header::AUTHORIZATION, format!("Bearer {}", token)).await;
...
async fn test_delete(pool: PgPool) {
	...
	let res = server.delete(&format!("/rooms/{}", room.id)).add_header(axum::http::header::AUTHORIZATION, format!("Bearer {}", token)).await;

	res.assert_status(StatusCode::NO_CONTENT);

	// Verify it's deleted
	let get_res = server.get(&format!("/rooms/{}", room.id)).add_header(axum::http::header::AUTHORIZATION, format!("Bearer {}", token)).await;
...
async fn test_publish_room(pool: PgPool) {
	...
	let res = server.post(&format!("/rooms/{}/publish", room.id)).add_header(axum::http::header::AUTHORIZATION, format!("Bearer {}", token)).await;
...
async fn test_privatize_room(pool: PgPool) {
	...
	server.post(&format!("/rooms/{}/publish", room.id)).add_header(axum::http::header::AUTHORIZATION, format!("Bearer {}", token)).await;

	let res = server.post(&format!("/rooms/{}/privatize", room.id)).add_header(axum::http::header::AUTHORIZATION, format!("Bearer {}", token)).await;
...
async fn test_transfer_ownership(pool: PgPool) {
	...
	let create_res = server.post("/rooms").add_header(axum::http::header::AUTHORIZATION, format!("Bearer {}", token1)).await;
	let room = create_res.json::<TestRoomResponse>();

	let res = server.post(&format!("/rooms/{}/transfer-ownership", room.id)).add_header(axum::http::header::AUTHORIZATION, format!("Bearer {}", token1)).json(&json!({ "new_owner_id": user2.id })).await;

	res.assert_status(StatusCode::NO_CONTENT);

	let get_res = server.get(&format!("/rooms/{}", room.id)).add_header(axum::http::header::AUTHORIZATION, format!("Bearer {}", token1)).await;
