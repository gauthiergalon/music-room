...
async fn test_queue_add(pool: PgPool) {
	...
	// Add track to queue
	let res = server.post(&format!("/rooms/{}/queue", room.id)).add_header(axum::http::header::AUTHORIZATION, format!("Bearer {}", token)).json(&json!({ "track_id": 12345 })).await;
...
async fn test_queue_get(pool: PgPool) {
	...
	// Add tracks to queue
	server.post(&format!("/rooms/{}/queue", room.id)).add_header(axum::http::header::AUTHORIZATION, format!("Bearer {}", token)).json(&json!({ "track_id": 111 })).await;
	server.post(&format!("/rooms/{}/queue", room.id)).add_header(axum::http::header::AUTHORIZATION, format!("Bearer {}", token)).json(&json!({ "track_id": 222 })).await;

	// Get queue
	let get_res = server.get(&format!("/rooms/{}/queue", room.id)).add_header(axum::http::header::AUTHORIZATION, format!("Bearer {}", token)).await;
...
async fn test_queue_remove(pool: PgPool) {
	...
	// Add track to queue
	server.post(&format!("/rooms/{}/queue", room.id)).add_header(axum::http::header::AUTHORIZATION, format!("Bearer {}", token)).json(&json!({ "track_id": 999 })).await;
	...
	// Remove track
	let del_res = server.delete(&format!("/rooms/{}/queue", room.id)).add_header(axum::http::header::AUTHORIZATION, format!("Bearer {}", token)).json(&json!({ "id": track_unique_id })).await;

	del_res.assert_status(StatusCode::NO_CONTENT);

	// Verify empty queue
	let get_res2 = server.get(&format!("/rooms/{}/queue", room.id)).add_header(axum::http::header::AUTHORIZATION, format!("Bearer {}", token)).await;
...
async fn test_queue_reorder(pool: PgPool) {
	...
	server.post(&format!("/rooms/{}/queue", room.id)).add_header(axum::http::header::AUTHORIZATION, format!("Bearer {}", token)).json(&json!({ "track_id": 111 })).await;
	server.post(&format!("/rooms/{}/queue", room.id)).add_header(axum::http::header::AUTHORIZATION, format!("Bearer {}", token)).json(&json!({ "track_id": 222 })).await;
	server.post(&format!("/rooms/{}/queue", room.id)).add_header(axum::http::header::AUTHORIZATION, format!("Bearer {}", token)).json(&json!({ "track_id": 333 })).await;
	...
	assert_eq!(reordered_q[1].position, 0.5, "Position not updated correctly");
}
